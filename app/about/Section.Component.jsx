...
const Section = (props) => {
  ...
  return (
    <section className="md:mt-3 overflow-x-hidden lg:overflow-clip h-[70vh] md:h-[60vh] rounded-none w-full overflow-scroll md:overflow-visible m-0 md:w-[80vw] md:m-auto grid grid-cols-[1fr] md:grid-cols-[1fr_1fr] grid-rows-[1fr_1fr] md:grid-rows-1 gap-y-24 md:gap-y-0 gap-x-12">
      <motion.div
      ...
      >
        <Image
          ...
          alt={'image'}
          className="w-full object-cover filter brightness-50 -z-50 object-center md:object-contain h-full"
        />
      </motion.div>
      <div
        className={`col-start-1 col-span-full md:col-start-${
          left ? '1' : '2'
        } filter brightness-200 self-center col-span-1 row-span-full grid grid-cols-[1fr] grid-rows-[min-content_min-content_min-content] gap-y-3`}
      >
        <motion.div
          initial={{ opacity: 0, scale: 0, x: 14 }}
          whileInView={{ opacity: 1, scale: 1, x: 0 }}
        ...
        >
          <p
            className={`text-[#6D6D6D] px-6 md:py-6 lg:py-9 xl:py-16 2xl:py:24 md:px-0 text-[18px] md:text-[20px] font-roboto leading-[34.84px]`}
          >
