...
import PicChange from './PicChange.Component'

function Tractor() {
  const [pic, setPic] = useState(0)

  const handlePicChange = (picNumber) => {
    setPic(picNumber)
  }
  ...
  return (
    <motion.section
      ...
      className={`transition-d w-screen grid grid-cols-1 grid-rows-[min-content_min-content]`}
    >
      <div
        className={`transition-d col-start-1 col-span-1 justify-self-center self-center row-start-1 row-span-1 group w-screen grid grid-cols-[1fr_1fr] grid-rows-1`}
      >
        <div
        ...
        <figure
          className={`transition-d h-[650px] group-hover:-z-50 w-full col-span-full row-start-1 row-span-1 group-hover:col-start-2 group-hover:col-span-1`}
        >
          <Image
            src={`/images/Business/Pics/Other${pic}.png`}
            width={100}
            height={100}
            alt={`Business Image`}
      ...
      <PicChange handlePicChange={handlePicChange} />
    </motion.section>
  )
}
